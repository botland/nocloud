...
export default function CheckoutModal({ cart, onClose, onOrderComplete, initialData, onDraftChange }: Props) {
  ...
  const preorderDepositVat = 0;
  const vatRateForDisplay = 0.2;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-2xl rounded-3xl bg-slate-950 border border-slate-800 shadow-2xl overflow-hidden">
        ...
        <div className="p-8 space-y-8">
          {/* Persistent Pre-order Notice - made more obvious */}
          {preorderMode && (
            <div className="p-5 border-2 border-amber-500/60 bg-amber-500/10 rounded-2xl text-sm text-slate-200 space-y-2 ring-1 ring-amber-500/30">
              <div className="flex items-center gap-x-2">
                <span className="text-amber-400">⚡</span>
                <p className="font-semibold text-amber-300 text-base">{t('preorderNoticeTitle')}</p>
              </div>
              ...
              {showVatBreakdown && (
                <p className="text-xs text-amber-400/80">{t('preorderVatNote', {
                  depositNet: preorderDeposit,
                  depositVat: preorderDepositVat,
                  rate: Math.round(vatRateForDisplay * 100),
                })}</p>
              )}
              <p className="text-[11px] text-amber-400/70 pt-1">Services (management, backup, etc.) start after the balance is settled and hardware ships.</p>
            </div>
          )}
          ...
          {step === 1 && (
            <div className="space-y-6">
              {/* form fields omitted for brevity in this push - full form remains unchanged */}
            </div>
          )}

          {step === 3 && (
            <>
              {/* Payment Method section - notice moved above for persistence */}
              <div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-800">
            ...
            {step < 3 ? (
              ...
              >
                {isSubmitting 
                  ? (preorderMode ? 'Creating your pre-order deposit session…' : t('processing')) 
                  : t('completeOrder')}
              </button>
            )}
          </div>
